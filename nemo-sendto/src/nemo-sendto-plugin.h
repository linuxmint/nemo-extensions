...
#ifndef _NEMO_SENDTO_PLUGIN_H_
#define _NEMO_SENDTO_PLUGIN_H_

#include <gmodule.h>
#include <gtk/gtk.h>

/**
 * SECTION:nemo-sendto-plugin
 * @short_description: nemo-sento plug-in
 * @stability: Stable
 * @include: bluetooth-plugin.h
 *
 * Plug-ins can be used to extend nemo-sendto.
 **/
 ...
 * @NEMO_CAPS_NONE: No capabilities
 * @NEMO_CAPS_SEND_DIRECTORIES: The plugin can send whole directories without compression
 * @NEMO_CAPS_SEND_IMAGES: The plugin only sends images which could be resized
 *
 * Capabilities of the plugin.
 **/
typedef enum {
	NEMO_CAPS_NONE = 0,
	NEMO_CAPS_SEND_DIRECTORIES = 1 << 0,
	NEMO_CAPS_SEND_IMAGES = 1 << 1,
} NstPluginCapabilities;
 ...
 * @gettext_package: The domain to use to translate the description, %NULL if the plugin is part of nemo-sendto
 ...
 *
 * A structure representing a nemo-sendto plugin. You should also call NST_INIT_PLUGIN() on the plugin structure to export it.
 **/
 ...
 *
 * A structure as used in nemo-sendto.
 **/
struct _NstPlugin
{
	GModule *module;
 ...
 *
 * Call this on an #NstPluginInfo structure to make it available to nemo-sendto.
 **/
